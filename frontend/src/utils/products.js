...
const placeholderImages = {
  ...
  glasses:
    "https://images.unsplash.com/photo-1608270861620-7191c1a877e6?w=300&h=300&fit=crop",
  spoons:
    "https://images.unsplash.com/photo-1610701596007-11502861dcfa?w=300&h=300&fit=crop",
  cutlery:
    "https://images.unsplash.com/photo-1610701596007-11502861dcfa?w=300&h=300&fit=crop",
...
export const getFallbackProducts = () => {
  // Return local data when API fails
  console.log(
    "📌 [PRODUCTS] Using fallback - showing locally cached products"
  );

  return fallbackProducts.map(
    (product, index) =>
...
export const fetchProducts = async () => {
  try {
    ...
    // Only use products from database
    const dbProducts = data.products || [];

    if (!Array.isArray(dbProducts)) {
      console.error(
        "❌ [PRODUCTS] Invalid products format from API"
      );
    ...
    if (dbProducts.length === 0) {
      console.warn(
        "⚠️ [PRODUCTS] Database is empty - using fallback"
      );
      return getFallbackProducts();
    }
    ...
    console.log(
      `✅ [PRODUCTS] Loaded ${normalizedProducts.length} products from database`
    );

    return normalizedProducts;
  } catch (error) {
    console.warn(
      "⚠️ [PRODUCTS] Failed to fetch from API:",
      error.message
    );

    console.log(
      "📌 [PRODUCTS] Falling back to local data..."
    );
